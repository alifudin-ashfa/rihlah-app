...
export default function VendorPage({ app }) {
  ...
  const vendorBillsSection = (
    <Section title="Tagihan Vendor" subtitle={canEdit ? "Daftar kebutuhan biaya atau anggaran yang akan dibayar ke vendor." : undefined}>
      <div className={canEdit ? "grid gap-6 2xl:grid-cols-[0.88fr_1.12fr]" : "grid gap-6"}>
        {canEdit ? (
        <div className="space-y-4 rounded-2xl border bg-slate-50 p-4">
          <div className="grid gap-3 lg:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <input className={inputClass} type="number" inputMode="numeric" min="0" placeholder="Contoh: 8000000" value={expenseForm.nominal} onChange={(event) => setExpenseForm((prev) => ({ ...prev, nominal: event.target.value }))} />
            </div>
            <div className="space-y-2 md:col-span-2">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            ...
            {canEdit && editingExpenseId ? <button onClick={resetExpenseForm} className={buttonOutline}>Batal edit</button> : null}
          </div>
        </div>
        ) : null}
  ...
  const vendorPaymentsSection = (
    <Section title="Pembayaran Vendor" subtitle={canEdit ? "Catat DP, cicilan, atau pelunasan dan simpan bukti transfer seperlunya." : undefined}>
      <div className={canEdit ? "grid gap-6 2xl:grid-cols-[0.88fr_1.12fr]" : "grid gap-6"}>
        {canEdit ? (
        <div className="space-y-4 rounded-2xl border bg-slate-50 p-4">
          <div className="grid gap-3 lg:grid-cols-2">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-700">Nominal</label>
              <input className={inputClass} type="number" inputMode="numeric" min="0" placeholder="Contoh: 2500000" value={vendorPaymentForm.nominal} onChange={(event) => setVendorPaymentForm((prev) => ({ ...prev, nominal: event.target.value }))} />
            </div>
          </div>
          ...
          {showVendorPaymentAdvanced ? (
            <div className="grid gap-3 lg:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <input className={inputClass} type="number" inputMode="numeric" min="0" placeholder="Opsional" value={vendorPaymentForm.biayaAdmin} onChange={(event) => setVendorPaymentForm((prev) => ({ ...prev, biayaAdmin: event.target.value }))} />
              </div>
              <div className="space-y-2 md:col-span-2">
                <label className="text-sm font-medium text-slate-700">Catatan</label>
              ...
              <div className="space-y-2 md:col-span-2">
                ...
                {canEdit ? <input ref={paymentProofInputRef} type="file" accept="image/*" className={inputClass} onChange={handleVendorProofUpload} /> : <p className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-500">Mode pelihat: upload bukti disembunyikan.</p>}
