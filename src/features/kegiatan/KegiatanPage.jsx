...
export default function KegiatanPage({ app }) {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className={canEdit ? "grid gap-6 2xl:grid-cols-[1.05fr_0.95fr]" : "grid gap-6 lg:grid-cols-2"}>
        {canEdit ? (
          <Section id="kegiatan" title="Pengaturan kegiatan" subtitle="Isi hanya data dasar yang dipakai berulang oleh seluruh halaman.">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2 md:col-span-2">
                ...
                <input className={inputClass} disabled={!canEdit} value={config.namaKegiatan} onChange={(event) => handleConfigChange("namaKegiatan", event.target.value)} />
              </div>

              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-700">Jumlah pembimbing</label>
                <input className={inputClass} disabled={!canEdit} type="number" inputMode="numeric" min="0" value={config.jumlahPembimbing} onChange={(event) => handleConfigChange("jumlahPembimbing", event.target.value)} />
              </div>

              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-700">Iuran default santri</label>
                <input className={inputClass} disabled={!canEdit} type="number" inputMode="numeric" min="0" value={config.iuranDefaultSantri} onChange={(event) => handleConfigChange("iuranDefaultSantri", event.target.value)} />
              </div>

              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-700">Akun tujuan</label>
                <input className={inputClass} disabled={!canEdit} placeholder="Contoh: Bendahara Kegiatan" value={config.akunTujuan} onChange={(event) => handleConfigChange("akunTujuan", event.target.value)} />
              </div>

              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-700">Nomor rekening / tujuan transfer</label>
                <input className={inputClass} disabled={!canEdit} placeholder="Contoh: BSI 123456789 a.n. Panitia" value={config.rekeningTujuan} onChange={(event) => handleConfigChange("rekeningTujuan", event.target.value)} />
              </div>

              <div className="space-y-2 md:col-span-2">
                <label className="text-sm font-medium text-slate-700">Catatan kegiatan</label>
                <textarea className={textAreaClass} disabled={!canEdit} placeholder="Informasi tambahan untuk bendahara atau panitia" value={config.catatanKegiatan} onChange={(event) => handleConfigChange("catatanKegiatan", event.target.value)} />
              </div>
            </div>

            <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
              {canEdit ? <button onClick={applyDefaultTargetToAll} className={buttonOutline}>Samakan target semua santri</button> : null}
              <Pill tone="sky">Total peserta {jumlahPeserta} orang</Pill>
            </div>
          </Section>
        ) : null}

        <Section title="Target Pendanaan" subtitle={canEdit ? "Pisahkan target iuran, dana yang sudah masuk, dan dana yang masih perlu dicari." : undefined}>
      ...
      <Section title="Pemasukan Lain" subtitle={canEdit ? "Iuran santri dihitung otomatis dari transaksi santri. Tambahkan hanya pemasukan di luar iuran." : undefined}>
        <div className={canEdit ? "grid gap-6 2xl:grid-cols-[0.9fr_1.1fr]" : "grid gap-6"}>
          {canEdit ? (
          <div className="space-y-4 rounded-2xl border bg-slate-50 p-4">
            <div className="grid gap-3 lg:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <input className={inputClass} type="number" inputMode="numeric" min="0" value={incomeForm.nominal} onChange={(event) => setIncomeForm((prev) => ({ ...prev, nominal: event.target.value }))} />
              </div>
            </div>
